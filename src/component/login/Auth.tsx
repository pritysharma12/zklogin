...
function AuthComponent() {
  ...
  const SignInWithGoogle = async () => {
    ...
    window.localStorage.setItem(KEY_PAIR_SESSION_STORAGE_KEY, ephemeralKeyPair.export().privateKey);
              ...
              <Button
                ...
                onClick={async () => {
                  await SignInWithGoogle();
                   const newNonce = localStorage.getItem("nonce") ?? "default_value";
              ...
              <Stack spacing={2}>
                ...
                <Box>
                  <Button
                    variant="contained"
                    onClick={() => {
                      if (!localStorage.getItem(KEY_PAIR_SESSION_STORAGE_KEY)) {
                        return;
                      }
                      const serializedKeyPair = window.localStorage.getItem(KEY_PAIR_SESSION_STORAGE_KEY);

                      if (serializedKeyPair) {
                        try {
                          const deserializedKeyPair: Ed25519Keypair = JSON.parse(serializedKeyPair);
                          
                          setEphemeralKeyPair(deserializedKeyPair);
                        } catch (error) {
                          console.error('Failed to parse and set the key pair:', error);
                        }
                      } else {
                        console.error('Ephemeral Key pair local storage item not found');
                      }
                ...
                <LoadingButton
                  ...
                  onClick={async () => {
                    try {
                      ...
                      setMaxEpoch(Number(localStorage.getItem(MAX_EPOCH_LOCAL_STORAGE_KEY)))
                      const zkProofResult = await axios.post(
                        SUI_PROVER_DEV_ENDPOINT,
                        {
                          ...
                          extendedEphemeralPublicKey:
                            extendedEphemeralPublicKey,
                          maxEpoch: maxEpoch,
                          jwtRandomness: localStorage.getItem(RANDOMNESS_SESSION_STORAGE_KEY),
                          salt: userSalt,
                          keyClaimName: "sub",
                        },
                        {
